...
function getip(){
    times=0;
    until [ `echo $ip | egrep -c "^(173|60|74|208|4)"` -eq 1 ]
    do
        ip=$(dig +tcp $1 @208.67.220.220 | egrep 'IN\s+?A'| tail -1 | awk '{printf("%s", $5)}')
        times=$(($times+1))
        if [ $times -eq 20 ]
        then
            break
...
do
    ip=$(grep "$host" hosts.do | awk "{if(\$2==\"$host\") printf(\"%s\", \$1);}")
    if [ -z $ip ]
    then
        ip=$(getip $host)
    fi
    ...
    output="$ip    $host"
    echo $output
    sed -i -E "s/.*?    $host/$output/" hosts.all
done
